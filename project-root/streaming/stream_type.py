...
class ReviewType:
    def __init__(self, row):
        ...
        self.reviewer_id = (
            int(row.get("reviewer_id")) if row.get("reviewer_id") else None
        )
        self.reviewer_name = row.get("reviewer_name")
        self.comments = row.get("comments")
        self.language = row.get("language")
        self.sentiment_score = (
            float(row.get("sentiment_score")) if row.get("sentiment_score") else None
        )
        self.sentiment_label = (
            [s.strip() for s in row.get("sentiment_label").split(",")]
            if row.get("sentiment_label")
            else None
        )
        self.likes_votes = (
            int(row.get("likes_votes")) if row.get("likes_votes") else None
        )
        self.event_ingestion_time = row.get("event_ingestion_time")
        self.raw_user_agent = row.get("raw_user_agent")
        self.bot_suspicion_score = (
            float(row.get("bot_suspicion_score"))
            if row.get("bot_suspicion_score")
            else None
        )
        ...
        self.comment_character_count = (
            int(row.get("comment_character_count"))
            if row.get("comment_character_count")
            else None
        )
        self.readability_index = (
            float(row.get("readability_index"))
            if row.get("readability_index")
            else None
        )
        self.session_id = row.get("session_id")
        self.time_spent_on_review_ms = (
            int(row.get("time_spent_on_review_ms"))
            if row.get("time_spent_on_review_ms")
            else None
        )
        self.contains_media = (
            str(row.get("contains_media")).lower() in ["true", "1", "yes"]
